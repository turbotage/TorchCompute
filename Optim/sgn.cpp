...
void tc::optim::SGN::step()
{
	...
	torch::Tensor chol_mask = step_mask == MaskTypes::SUCCESSFUL_CHOLESKY_GN;
	pD.index_put_({ chol_mask, Slice() },
		torch::bmm(D.index({chol_mask, Slice()}), torch::cholesky_solve(-gs.index({chol_mask, Slice()}), Hs_chol.index({chol_mask, Slice()}))));
	...
	pD.index_put_({ chol_mask, Slice() }, -gd_damp * torch::bmm(
		torch::diag_embed(Jn.index({ chol_mask, Slice()})),
		gs.index({ chol_mask, Slice()})));
